...
- BK7259 下载和串口日志均使用 :strong:`USB-UART` 接口。
- BK3515NS 下载使用 :strong:`USB-UART` 接口，需按上表切换对应跳线，仅下载连接，下载完成请拔掉。
- MTP 使用开发板的 :strong:`USB` 接口；连接 PC 后，在串口 CLI 执行 ``ap_cmd mtp start`` 以启用 SD 卡媒体设备。
