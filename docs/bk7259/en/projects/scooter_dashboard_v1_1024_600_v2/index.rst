...
- Use the :strong:`USB-UART` interface for BK7259 flashing and serial logs.
- Use the :strong:`USB-UART` interface for BK3515NS flashing. Switch the related jumpers as listed above, and disconnect the flashing connection after flashing is complete.
- Use the board's :strong:`USB` interface for MTP. After connecting to a PC, run ``ap_cmd mtp start`` through the serial CLI to expose the SD card as a media device.
