...
typedef struct {
    ...
    uint8_t dec_error;
} doorbell_pipeline_ctx_t;

static doorbell_pipeline_ctx_t *s_doorbell_pipeline = NULL;
...
static void doorbell_encode_flexa_done_cb(uint32_t done_lines, void *args)
{
}

static void doorbell_pipeline_decode_line_done_callback(uint32_t wr_cnt, void *arg)
{
}

static void doorbell_pipeline_decode_notify_callback(void *arg)
{
    doorbell_pipeline_ctx_t *ctx = (doorbell_pipeline_ctx_t *)arg;
...
avdk_err_t doorbell_h264_encode_open(uint16_t width, uint16_t height)
{
    avdk_err_t ret = AVDK_ERR_OK;

    if (s_doorbell_pipeline != NULL) {
        LOGW("doorbell h264 encode already running\n");
        return AVDK_ERR_OK;
    }

    doorbell_pipeline_ctx_t *ctx = (doorbell_pipeline_ctx_t *)os_malloc(sizeof(doorbell_pipeline_ctx_t));
    if (ctx == NULL) {
     ...
     */
    s_doorbell_pipeline = ctx;

    uint8_t *decode_buf = NULL;
    uint8_t ring_cnt = 0;
    ret = doorbell_decode_get_flexa_context(&decode_buf, &ring_cnt);
    ...
    enc_config.flexa_done = doorbell_encode_flexa_done_cb;
...
fail:
    if (ctx) {
        ctx->running = 0;
        if (s_doorbell_pipeline == ctx) {
            s_doorbell_pipeline = NULL;
        }
...
avdk_err_t doorbell_h264_encode_close(void)
{
    doorbell_pipeline_ctx_t *ctx = s_doorbell_pipeline;
    if (ctx == NULL) {
        return AVDK_ERR_OK;
    }
...
avdk_err_t doorbell_h264_encode_get_handle(bk_h264_encode_ctlr_handle_t *handle)
{
    if (s_doorbell_pipeline == NULL) {
        return AVDK_ERR_GENERIC;
    }
    *handle = s_doorbell_pipeline->enc_ctlr_handle;
    return AVDK_ERR_OK;
}