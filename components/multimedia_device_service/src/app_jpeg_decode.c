...
} doorbell_decode_ctx_t;

static doorbell_decode_ctx_t *s_doorbell_decode_ctx = NULL;
...
static void doorbell_jpeg_frame_done_cb(int status, void *args)
{
    (void)status;
    if (status != BK_OK) {
...
static void app_decode_thread_entry(void *arg)
{
    ...
    doorbell_decode_ctx_t *decoder_config = (doorbell_decode_ctx_t *)arg;
                                ...
                                uint8_t flexa_mode)
{
    ...
    doorbell_decode_ctx_t *decoder_config = s_doorbell_decode_ctx;
    ...
    decoder_config = (doorbell_decode_ctx_t *)os_malloc(sizeof(doorbell_decode_ctx_t));
    if (decoder_config == NULL) {
        LOGE("%s, %d, malloc decoder config failed\n", __func__, __LINE__);
        return AVDK_ERR_NOMEM;
    }

    os_memset(decoder_config, 0, sizeof(doorbell_decode_ctx_t));
    ...
    s_doorbell_decode_ctx = decoder_config;
...
out:
    if (decoder_config) {
        ...
        s_doorbell_decode_ctx = NULL;
    }

    return ret;
}

avdk_err_t doorbell_jpeg_decode_close(void)
{
    doorbell_decode_ctx_t *decoder_config = s_doorbell_decode_ctx;
...
avdk_err_t doorbell_decode_get_flexa_context(uint8_t **decode_buffer, uint8_t *ring_buffer_cnt)
{
    doorbell_decode_ctx_t *decoder_config = s_doorbell_decode_ctx;
    if (decoder_config == NULL) {
...
avdk_err_t doorbell_decode_get_handle(bk_jpeg_decode_ctlr_handle_t *handle)
{
    if (s_doorbell_decode_ctx == NULL) {
        return AVDK_ERR_GENERIC;
    }
    *handle = s_doorbell_decode_ctx->decode_handle;
    return AVDK_ERR_OK;
}