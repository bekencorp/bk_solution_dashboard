...
static void *doorbell_frame_malloc(uint32_t size)
{
...
static void doorbell_frame_complete(void *frame, uint32_t frame_size, void *args)
{
    (void)frame_size;
    (void)args;
    avdk_err_t ret = app_mipi_lcd_flush(frame, doorbell_frame_free);
    if (ret != AVDK_ERR_OK)
    {
...
avdk_err_t app_gpu_turn_on(gpu_board_config_t *config)
{
    ...
    gpu_config.frame_malloc = doorbell_frame_malloc;
    gpu_config.frame_free = doorbell_frame_free;
    gpu_config.flexa_line_done = NULL;
    gpu_config.flexa_line_done_args = NULL;
    gpu_config.frame_done = doorbell_frame_complete;
