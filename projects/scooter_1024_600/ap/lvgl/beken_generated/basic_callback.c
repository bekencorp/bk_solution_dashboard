...
void lv_mileage_timer_cb(lv_timer_t *timer)
{
    ...
    lv_spangroup_set_span_text(bk_lv_tool_ui.page_1_spangroup_1, bk_lv_tool_ui.page_1_spangroup_1_span_0, trip);

    if (fmod(trip_value, 1.0) < 0.1) {
        mileage_value++;
        sprintf(mileage, "%d", (int)mileage_value);
        lv_spangroup_set_span_text(bk_lv_tool_ui.page_1_spangroup_2, bk_lv_tool_ui.page_1_spangroup_2_span_0, mileage);
    }
...
void lv_speed_timer_cb(lv_timer_t *timer)
{
    ...
    lv_label_set_text(bk_lv_tool_ui.page_1_label_3, speed);
    lv_scale_set_line_needle_value(bk_lv_tool_ui.page_1_scale_1, bk_lv_tool_ui.page_1_scale_1_needle_0, 91, value);
}
...
void lv_turn_signal_timer_cb(lv_timer_t *timer)
{
    ...
    if (lv_turn_signal_is_green) {
        lv_image_set_src(signal_obj, &light_grey_98x101_RGB565A8_NONE);
        lv_turn_signal_is_green = false;
    } else {
        lv_image_set_src(signal_obj, &light_green_96x99_RGB565A8_NONE);
