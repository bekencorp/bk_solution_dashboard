...
本工程固定用于 BK7259 + FL7703 新屏板卡。ER68576B 老屏使用独立的
`scooter_dashboard_klok_1280_720` 工程，两套工程不通过宏切换。
目前没有经过确认的 FL7703 硬件原理图，其引脚配置仅依据可用固件
分析结果和实际硬件测试结果。
...
- MIPI 面板固定为 FL7703，物理分辨率为 720x1280，LVGL
  旋转后使用 1280x720 逻辑分辨率。
- 触摸固定使用项目级 GT911：RST=P40、INT=P41、SDA=P1、SCL=P0，
  并启用 IRQ + 轮询回退。
- SD 卡固定使用 SDIO0：P2/P3/P4、20 MHz、1-bit。
