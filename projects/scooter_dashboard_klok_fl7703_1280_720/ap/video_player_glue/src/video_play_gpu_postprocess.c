...
void video_play_gpu_postprocess_release_lvgl_context(void)
{
    ...
    /*
     * The PP-OSD path borrows LVGL's VG-Lite context, but its strip DMA
     * buffers occupy the same scarce HSRAM needed by the Flexa GPU worker.
     * Release them when leaving PP-OSD before a KTV Flexa pipeline is opened.
     */
    s_gpu_post_reserved_for_lvgl = false;
    video_play_gpu_postprocess_deinit_locked();
    video_play_gpu_post_unlock();
}
