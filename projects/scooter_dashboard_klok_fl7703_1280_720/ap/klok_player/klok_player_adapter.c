...
static volatile bool s_repeat_current = false;
...
{
#if CONFIG_BT
    if (bk_avrcp_ct_is_connected()) {
        (void)bk_avrcp_ct_pause();
    }
    /*
     * Stop accepting A2DP PCM immediately. Waiting for a remote SUSPEND is not
     * reliable, and the local stop acknowledgement already serializes source
     * detach before KTV attaches to the same shared speaker.
     */
    a2dp_sink_demo_audio_spk_enable(0);
    /*
     * A format change on a still-running speaker pauses the element, but the
     * driver's open_cnt workaround then skips restarting its DMA. Destroy the
     * now-idle service so KTV gets a fresh speaker instance and open_cnt/DMA
     * state are reset together. The A2DP acknowledgement above guarantees its
     * source has already detached.
     */
    if (spk_service_deinit() != BK_OK) {
        bk_printf("klok player: shared speaker reset failed\r\n");
    }
#endif
}
...
static void klok_player_on_finished(const char *file_path, void *user_data)
{
    ...
    /*
     * Restart directly from the player's finish notification thread. This is
     * the same callback context used by the SDK playlist REPEAT mode and
     * avoids waiting for the UI's 500 ms polling timer. The preview image keeps
     * the final frame until the first frame of the next pass is ready.
     */
    if (s_repeat_current &&
        klok_player_start_file(s_current_file_path) == 0) {
        bk_printf("klok player: seamless repeat: %s\r\n",
                  s_current_file_path);
        return;
    }
...
static int klok_player_start_file(const char *file_path)
{
    ...
    if (ret == 0) {
        ...
        /*
         * A2DP and KTV program the same DAC through different audio stacks.
         * Re-apply KTV's PCM format after an A2DP teardown, then explicitly
         * restore both gain and mute state instead of relying on reset values.
         */
        (void)video_play_engine_api_reassert_audio_format();
        (void)video_play_engine_api_set_volume(s_volume);
        (void)video_play_engine_api_set_mute(s_muted);
    }
...
{
    return klok_player_set_muted(!s_muted);
}

int klok_player_set_muted(bool muted)
{
    bool previous = s_muted;
    s_muted = muted;
    if (!s_started) {
        return 0;
    }
