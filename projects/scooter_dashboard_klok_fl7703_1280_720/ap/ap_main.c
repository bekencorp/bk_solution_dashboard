...
#include <lcd/lcd_mipi_fl7703_720x1280.h>
#include "klok_board_profile.h"

#define KLOK_MIPI_PANEL_DEVICE lcd_device_fl7703_mipi_720x1280
...
{
    /*
     * UART1 is remapped to P38/P39 so P0/P1 remains available for the
     * FL7703 profile's touch I2C bus.
     */
    bsc_config_t cfg = {
...
{
    /* The working reference BIN does not control an SD power GPIO. */
    bk_printf("SD profile: host=0 bus=1-bit pins=P2/P3/P4 power=external\r\n");
}
...
static void display_board_backlight_enable(void)
{
    static const gpio_id_t backlight_pins[] = {GPIO_7, GPIO_28};

    for (size_t i = 0; i < sizeof(backlight_pins) / sizeof(backlight_pins[0]); i++) {
        gpio_id_t pin = backlight_pins[i];
        gpio_dev_unmap(pin);
    ...
    bk_printf("LCD controls: P%d=%d P%d=%d\r\n",
              backlight_pins[0], bk_gpio_get_output(backlight_pins[0]),
              backlight_pins[1], bk_gpio_get_output(backlight_pins[1]));
}
...
static void lvgl_ui_init(void)
{
    ...
    /* FL7703 uses two board controls whose exact split is not documented. */
