...
本工程面向 Dashboard V1.0 硬件，包含主页导航、Dashcam 循环录像与录像回放、电话簿、Assist View、蓝牙电话控制、网络配网以及 SD 卡 MTP 导出功能。
...
| 页面 | 用途 | 进入方式 |
| --- | --- | --- |
| 主页（Home） | 系统默认页面，用于选择 Dashcam、OTA 或电话簿 | 上电后默认进入；在功能页面中双击中键返回 |
...
| 按键 | GPIO | 操作 | 功能 |
| --- | --- | --- | --- |
| 上 | GPIO32 | 短按 | 电话簿当前列表中选择上一项 |
| 下 | GPIO27 | 短按 | 电话簿当前列表中选择下一项 |
| 下 | GPIO27 | 长按 | 仅在主页清除已保存的网络配网信息并重启 |
| 左 | GPIO31 | 短按 | 选择上一项；电话簿中切换到联系人列表 |
| 左 | GPIO31 | 长按 | 仅在主页打开 Assist View |
| 右 | GPIO29 | 短按 | 选择下一项；电话簿中切换到通话记录列表 |
| 右 | GPIO29 | 长按 | 仅在主页打开 Assist View |
| 中 | GPIO30 | 短按 | 确认当前选项：进入页面、播放录像、拨打电话或执行来电/通话操作 |
...
| --- | --- | --- | --- |
| 主页 | 左/右键在 Dashcam、OTA、电话簿之间切换 | 进入当前选中的页面 | 保持或返回主页 |
