 ...
 * reaches from that corner to the far end of the needle - up to 380x380 on these
 * 400x400 gauges - and lv_line_set_points_mutable() invalidates it twice, once
 * for the old size and once for the new. Two gauges stepping every 10ms measured
 * 269k pixels per frame, and 63ms of LVGL render time in an 83ms frame.
 *
 ...
 *
 * Both are laid out at the full panel width - 465 and 475 - with
 * LV_TEXT_ALIGN_CENTER, so a one or two digit number centres itself inside a
 * 98px band spanning the panel, and lv_label_refr_text() invalidates all of it
 * on every update. That is 92k pixels a frame between the two, and it was free
 * only while the needle's bounding box still covered the whole gauge and
 * swallowed it - shrinking that box above is what exposes this one.
 *
 ...
 * the axis the text was already centred on, and holding y at 181 leaves the
