 ...
 * and largest free block, not bytes used, is the number that actually tells
 * you whether an allocation is about to fail. display_ui_lv_pool_alloc()
 * (components/display_ui) takes the whole block from HSRAM here, because this project
 * sets CONFIG_LVGL_MEM_USE_PSRAM=n.
 *
 ...
 *
 * 170 KiB is carried over from the 1024x600 project, whose peak after the same
 * four memory changes measured 139 KiB. This build has not been profiled yet,
 * and its UI is the larger of the two, so treat the number as provisional:
 * watch for "HSRAM LVGL pool alloc failed" and raise it. The line right after
 * it, "LVGL pool fell back to PSRAM", means the UI is alive but running out of
 * the slow heap.
 *
 ...
 * whole LV_MEM_SIZE block goes back to HSRAM and the next lv_init() takes a
 * fresh one that may well sit elsewhere. Every pointer allocated before an
 * lv_deinit() therefore dangles. Assist view and clip playback both run that
 * cycle (display_ui_deinit_lvgl() is unconditional in dashcam_assitview.c and
 * dashcam_video_v4.c), so every module that caches an LVGL handle drops it in
 * its *_reset_after_lvgl_deinit(); see beken_ui_after_lvgl_deinit(), which
 * both paths now call.
 */
 ...
 * the 1024x600 build found 298 live entries holding 15KB, as ~30-byte blocks
 * scattered across the pool. 0 is a supported size, not a trick:
 * ttf_get_glyph_pair_kerning_width computes on the stack and returns when
 * max_size is 0. Raise this again if a font that actually needs kerning is
 * ever added. */
