...
static void phone_book_update_header(void)
{
    ...
    if (ui->phone_book_bt_ic != NULL && lv_obj_is_valid(ui->phone_book_bt_ic))
    {
        lv_image_set_src(ui->phone_book_bt_ic,
                         connected ? &bt_sm_28x28_RGB565A8_NONE
                                   : &bt_gray_sm_28x28_RGB565A8_NONE);
    }
                                       ...
                                       const char *number, lv_font_t *cn)
{
    ...
    lv_image_set_src(av, &avatar_sm_52x52_RGB565A8_NONE);
    ...
    lv_obj_set_style_text_font(nm, cn ? cn : &lv_font_montserrat_regular_32,
                               LV_PART_MAIN | LV_STATE_DEFAULT);
...
static const lv_image_dsc_t *recent_type_icon(uint8_t type)
{
    switch (type)
    {
    case PBAP_CALL_RECEIVED: return &arrow_in_sm_30x30_RGB565A8_NONE;
    case PBAP_CALL_MISSED:   return &arrow_miss_sm_30x30_RGB565A8_NONE;
    case PBAP_CALL_DIALED:
    default:                 return &arrow_out_sm_30x30_RGB565A8_NONE;
    }
}
