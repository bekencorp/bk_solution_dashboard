                                         ...
                                         video_player_buffer_t *buffer)
{
    ...
    /* The HW H264 frame decoder only emits NV12; convert to RGB565 for the
     * LVGL canvas inside dashcam_video_on_frame. */
    if (meta != NULL && buffer != NULL &&
        buffer->data != NULL &&
        (meta->output_format == PIXEL_FMT_NV12 ||
         meta->output_format == PIXEL_FMT_YUV420SP))
    {
        dashcam_video_on_frame(buffer->data,
                               meta->video.width,
                               meta->video.height,
