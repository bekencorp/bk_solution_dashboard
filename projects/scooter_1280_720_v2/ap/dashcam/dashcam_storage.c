...
void dashcam_storage_format_label(const dashcam_file_info_t *info, char *buf, size_t size)
{
    ...
    /*
     * Plan B (dev/test): no wall clock available, so show sequence index plus
     * size. The boot-relative uptime is also encoded to keep ordering stable
     * within a single power cycle.
     */
    uint32_t size_kb = info->size_bytes / 1024u;
    if (size_kb >= 1024u)
    {
        snprintf(buf, size, "REC %03u  %u.%uMB",
                 (unsigned)info->seq,
                 (unsigned)(size_kb / 1024u),
                 (unsigned)((size_kb % 1024u) * 10u / 1024u));
    }
    else
    {
