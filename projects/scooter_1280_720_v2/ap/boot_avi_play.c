...
/* Set by boot_bg_preload when it owns the shared SD mount (see boot_bg_preload.c). */
extern volatile bool g_boot_sd_pre_mounted;

static bk_err_t sdcard_mount(void)
{
#if CONFIG_VFS
    ...
    if (g_boot_sd_pre_mounted) {
        LOGI("SD already mounted by preloader, reuse\n");
        return BK_OK;
    }
...
{
    return bk_display_pixel_format_set(display_ui_get_dpu_handle(),
...
{
    const bk_display_dpu_config_t *cfg = display_ui_get_dpu_config();

    return bk_display_pixel_format_set(display_ui_get_dpu_handle(),
                                       &(bk_display_pixel_format_config_t) {
                                           .format = cfg->video.format,
                                           .decompress = cfg->video.decompress,
                                       });
}
