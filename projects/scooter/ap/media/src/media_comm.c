...
int av_server_socket_sendto(int *fd, const struct sockaddr *dst, uint8_t *data, uint32_t length, int offset)
{
	int ret = 0;

	uint8_t *ptr = data + offset;
	uint16_t size = length - offset;
	int max_retry = AV_SERVER_SEND_MAX_RETRY;
	uint16_t index = 0;
	...
	do {
		...
		rtos_delay_milliseconds(AV_SERVER_SEND_MAX_DELAY);
