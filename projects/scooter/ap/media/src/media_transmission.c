...
void av_server_hex_dump(uint8_t *data, uint32_t length)
{
#ifdef DUMP_DEBUG
	for (int i = 0; i < 12; i++)
	{
...
db_channel_t *av_server_transmission_malloc(uint16_t max_rx_size, uint16_t max_tx_size)
{
...
void av_server_transmission_unpack(db_channel_t *channel, uint8_t *data, uint32_t length, av_server_transmission_recive_cb_t cb)
{
	...
	av_server_hex_dump(data, length);
#else
	LOGV("recv unpack: %u\n", length);
#endif
	...
	while (left != 0)
	{
		...
		else
		{
			...
			else
			{
				...
				av_server_hex_dump(channel->cbuf, channel->ccount);
				//TODO FIXME
				break;
			}
...
uint32_t av_server_transmission_get_milliseconds(void)
{
	uint32_t time = 0;
...
void av_server_transmission_pack(db_channel_t *channel, uint8_t *data, uint32_t length)
{
	...
	*/
	...
	head->timestamp = CHECK_ENDIAN_UINT32(av_server_transmission_get_milliseconds());
...
int av_server_transmission_pack_send(db_channel_t *channel, uint8_t *data, uint32_t length, av_server_transmission_send_t cb)
{
	bk_err_t ret = BK_FAIL;

	av_server_transmission_pack(channel, data, length);
