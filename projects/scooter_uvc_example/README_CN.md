...
本工程（`scooter_uvc_example`）主要用于测试 UVC 功能（设备接入、预览显示链路、帧队列与解码显示流程）。

* [English](./README.md)

## 1 项目概述

本工程（`scooter_uvc_example`）主要面向 UVC 功能联调与验证：支持 UVC 摄像头接入与预览显示（MJPEG -> JPEG 解码 -> LVGL 显示），并保留 WiFi 图传、LVGL UI、蓝牙等能力以便进行系统级联调。
...
- 支持UVC预览显示，通过LVGL view在“导航/ UVC / 默认界面”间切换
- AVI视频播放（可选）
...
4. （可选）连接UVC摄像头，进入UVC预览模式，在LVGL的UVC界面显示摄像头画面
...
### 4.4 UVC预览模式命令（UVC -> media_frame_queue -> JPEG解码 -> LVGL显示）

```
// 进入UVC预览模式（默认 480x320 MJPEG）
ap_cmd test uvc enter

// 进入UVC预览模式（指定MJPEG分辨率）
...
1. frame_buffer_que（frame_queue）：用于管理UVC摄像头输出的MJPEG/H264等图像帧（生产者侧通过 `frame_queue_malloc()` / `frame_queue_complete()` 入队，消费者通过 `frame_queue_get_frame()` 取帧）
2. media_frame_queue：用于管理“待JPEG解码显示”的完整帧数据（例如 WiFi 导航图像、UVC 预览的 MJPEG 帧）。解码完成后通过 `lvgl_app_display()` 根据当前 LVGL view 显示到对应界面

.. note::
    当前代码中，JPEG 解码线程固定从 `media_frame_queue_get_frame()` 取帧；是否将帧送入 `media_frame_queue` 由生产者侧根据当前 LVGL view（`lvgl_app_get_view()`）决定：

    - UVC 预览：仅当 view 为 `LVGL_VIEW_UVC` 时才将 MJPEG 帧入队，否则直接丢弃以降低解码/队列压力。
    - 导航/图传：仅当 view 为 `LVGL_VIEW_NAVIGATION` 时才将导航图像入队，否则直接丢弃以降低解码/队列压力。
...
#### 5.2.5 LVGL view与显示路由

本工程使用 `lvgl_app_get_view()` 管理当前显示界面（默认/导航/UVC）。解码完成后的图像统一调用 `lvgl_app_display()`，由其根据当前view路由到对应界面显示。
...
/**
 * @brief 显示一帧视频图像（自动按当前view路由）
 * @param data 图像数据（RGB565 或 YUYV）
 * @param data_len 数据长度
 * @param data_is_rgb565 true: data为RGB565；false: data为YUYV（内部会做YUYV->RGB565转换）
 */
...
/**
 * @brief 启动JPEG解码管理器
 * @details 解码线程会从 `media_frame_queue_get_frame()` 取帧解码，解码完成后调用 `lvgl_app_display()` 按当前LVGL view显示
 */
...
1. 确保手机与设备在同一WiFi网段
2. 设备IP地址可通过串口命令查看
3. 切到导航图像时会暂停LVGL界面显示（如使用 `ap_cmd test switch wifi` 切到WiFi图传显示）
