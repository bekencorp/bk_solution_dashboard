# Scooter Project UVC example

This project (`scooter_uvc_example`) is mainly used to test UVC functionality (device connection, preview pipeline, frame queues, and decode/display flow).

* [中文](./README_CN.md)

## 1 Project Overview

This project (`scooter_uvc_example`) mainly targets UVC bring-up and verification. It supports UVC camera preview (MJPEG -> JPEG decode -> LVGL display) and keeps WiFi image transfer, LVGL UI, Bluetooth, etc. for system integration testing.
...
- Support UVC preview display and switching between LVGL views (Default / Navigation / UVC)
...
4. (Optional) Connect a UVC camera and enter UVC preview mode to show MJPEG frames on the LVGL UVC page
...
### 4.4 UVC Preview Mode (UVC -> media_frame_queue -> JPEG decode -> LVGL)

```
// Enter UVC preview mode (default 480x320 MJPEG)
ap_cmd test uvc enter

// Enter UVC preview mode with specified MJPEG resolution
ap_cmd test uvc enter <width> <height>

// Exit UVC preview mode
ap_cmd test uvc exit
```

### 4.5 bluetooth ctrl cmd
...
This section describes frame buffer queue management APIs used in the scooter project. The project uses two queue families:

1. frame_buffer_que (frame_queue): used to manage UVC MJPEG/H.264 frames (producer uses `frame_queue_malloc()` / `frame_queue_complete()`, consumer uses `frame_queue_get_frame()`)
2. media_frame_queue: used to manage “complete frames waiting for JPEG decode & display” (e.g., WiFi navigation images and UVC preview MJPEG frames). After decoding, the output is displayed by `lvgl_app_display()` based on current LVGL view

.. note::
    In current code, the JPEG decode thread always fetches frames from `media_frame_queue_get_frame()`. Whether a frame is enqueued to `media_frame_queue` is decided by the producer side based on current LVGL view (`lvgl_app_get_view()`):

    - UVC preview: enqueue MJPEG frames only when view is `LVGL_VIEW_UVC`; otherwise drop frames to reduce decode/queue pressure.
    - Navigation/image transfer: enqueue frames only when view is `LVGL_VIEW_NAVIGATION`; otherwise drop frames to reduce decode/queue pressure.
...
#### 5.2.5 LVGL views and display routing

This project uses `lvgl_app_get_view()` to track the current LVGL view (Default / Navigation / UVC). Decoded frames call `lvgl_app_display()` and it routes to the correct view automatically.
...
/**
 * @brief Display one video frame (auto-routes by current view)
 * @param data image data (RGB565 or YUYV)
 * @param data_len data length
 * @param data_is_rgb565 true: RGB565; false: YUYV (internal YUYV->RGB565 conversion)
 */
...
```c
/**
 * @brief Enable JPEG decode manager
 * @details The decode thread consumes `media_frame_queue_get_frame()` and calls `lvgl_app_display()` after decoding
 */
