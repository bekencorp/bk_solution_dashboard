...
void cli_test_cmd(char *pcWriteBuffer, int xWriteBufferLen, int argc, char **argv)
{
    ...
    {
        /*
         * Recorder preview mode:
         * - Turn on JPEG decode manager (consumer of media_frame_queue)
         * - Open UVC camera (producer -> media_frame_queue_malloc/complete)
         *
         * Usage:
         ...
         *
         * Note:
         *   LVGL widget expects 480x320 RGB565/YUYV buffer (see lvgl_app_ui.c),
         *   so non-480x320 may not be displayed.
         */
        if (argc < 3)
        {
            LOGD("param error, %d\n", __LINE__);
        ...
        if (os_strcmp(argv[2], "enter") == 0)
        {
            bk_err_t bk_ret = BK_OK;

            /*
             * Ensure LVGL is running before switching pages / updating image widget.
             * Some users may have suspended LVGL by "ap_cmd test switch wifi".
             */
            lvgl_app_enter_uvc();

            /* Ensure device context is ready for decode manager */
            bk_ret = av_server_devices_init();
            if (bk_ret != BK_OK)
            {
                LOGE("%s %d av_server_devices_init failed:%d\n", __func__, __LINE__, bk_ret);
                ret = AVDK_ERR_GENERIC;
                goto out;
            }

            /* Initialize media frame queue for MJPEG input frames */
            ret = media_frame_queue_init(IMAGE_MJPEG);
            if (ret != AVDK_ERR_OK)
            {
                LOGE("%s %d media_frame_queue_init failed:%d\n", __func__, __LINE__, ret);
                lvgl_app_exit_navigation();
                goto out;
            }

            bk_ret = av_server_jpeg_decode_manager_turn_on();
            if (bk_ret != BK_OK)
            {
                LOGE("%s %d av_server_jpeg_decode_manager_turn_on failed:%d\n", __func__, __LINE__, bk_ret);
                ret = AVDK_ERR_GENERIC;
                lvgl_app_exit_uvc();
                goto out;
            }

            ret = uvc_camera_open_recorder(pcWriteBuffer, xWriteBufferLen, argc, argv);
            if (ret != AVDK_ERR_OK)
            {
                LOGE("%s %d uvc_camera_open_recorder failed:%d\n", __func__, __LINE__, ret);
                uvc_camera_close(pcWriteBuffer, xWriteBufferLen, argc, argv);
                av_server_jpeg_decode_manager_turn_off();
                lvgl_app_exit_uvc();
                goto out;
            }
        }
        else if (os_strcmp(argv[2], "exit") == 0)
        {
            bk_err_t bk_ret = BK_OK;

            /*
             * Stop producer first to avoid pushing frames after queue/decode manager is stopped.
             * Close UVC regardless of current mode.
             */
            (void)uvc_camera_close(pcWriteBuffer, xWriteBufferLen, argc, argv);

            bk_ret = av_server_jpeg_decode_manager_turn_off();
            if (bk_ret != BK_OK)
            {
                LOGW("%s %d av_server_jpeg_decode_manager_turn_off failed:%d\n", __func__, __LINE__, bk_ret);
            }
