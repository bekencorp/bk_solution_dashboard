# Scooter Dashboard V1.0 1024×600 V2 使用说明

* [English](./README.md)

## 开发板使用说明

本说明适用于搭载 1024×600 显示屏的 Dashboard V1.0 硬件，对应工程 `scooter_dashboard_v1_1024_600_v2`。
...
| LVGL 画布 | 1024×600 |
| UI 旋转 | 180° |
| 启动背景 | SD 卡中的 `/sd0/home_bg_tech_1024_600.jpg` |
| 投屏 JPEG | 输入/输出 1024×600，旋转 180° |
| Dashcam 采集与录像 | ISP MP 1280×720，H.264 录像 25 FPS |
| Assist View | 输出 1024×600，旋转 180°并缩放至屏幕 |
| 录像回放 | 输出 1024×600，旋转 180° |
| 分辨率资源 | 使用文件名带 `1024x600` 的背景图片及适配该分辨率的布局、图标和字体 |

本工程固件及 UI 资源仅适用于 1024×600 屏幕，请勿与 `scooter_dashboard_v1_1280_720_v2` 的固件或生成资源混用。
...
本工程面向 Dashboard V1.0 1024×600 硬件，包含主页导航、Dashcam 循环录像与录像回放、SD 卡本地音乐播放、电话簿、Assist View、蓝牙电话控制、网络配网以及 SD 卡 MTP 导出功能。
...
| 中 | GPIO30 | 长按 | 开关 ANCS BLE 广播（默认关闭）。打开后手机可搜索 `BK_DASHBOARD_*` 并配对通知；再长按关闭 |
