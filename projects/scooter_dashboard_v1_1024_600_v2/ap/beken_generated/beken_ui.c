...
/*
 * Page beken_ui_init() brings up. HOME at boot and after an Assist exit; clip
 * playback overrides it so the rebuild lands straight on the records list
 * instead of building and entering home only to switch away from it.
 */
 ...
 * lv_vendor_deinit() hands back to HSRAM wholesale - no per-allocation
 * lv_free() involved. Every pre-deinit allocation therefore points into memory
 * the system heap now owns and will re-issue to the decoder, the GPU or the
 * next pool. Reading one is a use-after-free, and lv_free()ing one corrupts
 * whoever holds that memory now (or aborts in lv_tlsf_free() with "block
 * already marked as free" when the new pool happens to land on top of it).
 *
...
/*
 * The rebuild runs beken_ui_init(), which brings HOME up and re-schedules the
 * dashcam boot, so the only thing beken_ui_kick_after_display_resume() used to
 * do that is still missing is re-arming the segment tick that
 * beken_ui_before_cast_lvgl_teardown() paused.
 */
