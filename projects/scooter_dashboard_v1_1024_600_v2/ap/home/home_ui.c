 ...
 *
 * Contains the home page's own behavior only (speed gauge, the music/phone
 * panel and its Bluetooth callbacks, and the home background bitmap).
 * Navigation between pages stays in beken_ui.c, mirroring the ota_ui /
 * dashcam_ui split.
 */
...
#define HOME_MUSIC_BEAT_MAX_HEIGHT     70
#define HOME_MUSIC_PANEL_X             348
#define HOME_MUSIC_PANEL_Y             8
#define HOME_MUSIC_BEAT_CANVAS_X       (HOME_MUSIC_PANEL_X + 16)
#define HOME_MUSIC_BEAT_CANVAS_Y       (HOME_MUSIC_PANEL_Y + 10)
#define HOME_MUSIC_BEAT_CANVAS_W       248
#define HOME_MUSIC_BEAT_CANVAS_H       70
#define HOME_MUSIC_BEAT_BAR_COUNT      20
#define HOME_MUSIC_BEAT_BAR_W          6
...
static bool home_music_beat_canvas_ready(void)
{
    ...
    {
        /* Keep this ~34KB canvas out of the tight SRAM heap: allocate it in PSRAM
...
void home_ui_enter(void)
{
     ...
     * (montserrat_regular_16) only covers ASCII 0x20-0x7F, so U+00D7 '×' renders
     * blank. Replace it with an ASCII 'x' so the multiplier shows. Kept here so a
